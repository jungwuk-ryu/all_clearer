      ...
      children: [
        Center(
          child: Shimmer.fromColors(
              ...
              child: Obx(() => Text(
                '${controller.time}',
                style: GoogleFonts.nanumGothicCoding(fontSize: 20, fontWeight: FontWeight.bold),
              ))),
        ),
            ...
            Column(
              children: [
                ...
                PaddingColumn(
                  children: [
                    GestureDetector(
                      onTap: () => controller.toTimeSetPage(NTPTimeSync('ubuntu.pool.ntp.org')),
                      child: const BorderContainer(
                          title: '🟠우분투', body: '많은 서버가 시간을 맞추기 위해 사용하는 서버예요'),
                    ),
                    GestureDetector(
                      onTap: () => controller.toTimeSetPage(NTPTimeSync("pool.ntp.org")),
                      child: const BorderContainer(
                          title: '⚫️pool.ntp.org', body: '전 세계적으로 분산된 시간 제공 서버'),
                    ),
                    GestureDetector(
                      onTap: () => controller.toTimeSetPage(NTPTimeSync("time.google.com")),
                      child:
                      const BorderContainer(title: '🔵Google', body: '구글이 제공하는 시간'),
                    ),
                    GestureDetector(
                      onTap: () => controller.toTimeSetPage(NTPTimeSync("time.windows.com")),
                      child: const BorderContainer(
                          title: '🟢Microsoft', body: '마이크로소프트가 제공하는 시간'),
                    ),
                    GestureDetector(
                      onTap: () => controller.toTimeSetPage(NTPTimeSync("time.apple.com")),
                      child:
                      const BorderContainer(title: '🔴Apple', body: '애플이 제공하는 시간'),
                    ),
                    GestureDetector(
